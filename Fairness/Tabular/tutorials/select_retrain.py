...
# begin retraining
X_retrain_1 = total_list_adf
X_retrain_2 = total_list_adf_2
Y_retrain = Y_total_list_adf

retrain_len = X_retrain_1.shape[0] 

stra = ['best']
acc_fp = [[] for i in range(len(stra))] 


for nums in [10]:
    for w in range(len(stra)):
        s = stra[w]
        
        X_additional = []
        Y_additional = []
        num_inputs_for_retrain = int(nums * 100)
        print("SELECT NUM: ", num_inputs_for_retrain, " Strategy:", s)
        ...
        if s == 'gini':
            indexes = select(ginis, num_inputs_for_retrain, s=s)
        else:
            indexes = select(fols, num_inputs_for_retrain, s=s)
